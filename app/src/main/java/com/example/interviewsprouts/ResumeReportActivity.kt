...
• Optional resume point rewrites
    ...
    private fun startAdvancedLoadingAnimation(textView: TextView) {
        ...
        advancedLoadingRunnable = object : Runnable {
            override fun run() {
                val dots = ".".repeat(advancedLoadingDotCount + 1)
                textView.text = "Generating Diamond Star Analysis$dots"
                advancedLoadingDotCount = (advancedLoadingDotCount + 1) % 3
...
${compactQuestions(generateInterviewQuestionsFromResume(resumeText, targetRole, experienceLevel, jobSpecification))}
    ...
    private fun createResumeReport(
        ...
        val overallScore = weightedOverallScore(
            jdRoleMatchScore = jdRoleMatchScore,
            ...
            toolSkillProofScore = toolSkillProofScore,
            candidatePositioningScore = candidatePositioningScore,
...
• Current analysis:
  This analysis is based on the selected target role.
            ...
            val foundJdText = foundJdKeywords
                .take(8)
                .joinToString("\n") { "  • $it" }
                .ifBlank { "  • No clear JD keyword matches detected." }

            val missingJdText = missingJdKeywords
                .take(8)
                .joinToString("\n") { "  • $it" }
                .ifBlank { "  • No major JD keyword gaps detected." }
  ...
  Replace vague lines with a specific task, tool used, and real outcome.
    ...
    private fun weightedOverallScore(
        ...
        if (resumeText.length < 300) {
            cappedScore = minOf(cappedScore, 55)
        }
        ...
        if (
            ...
            cappedScore = minOf(cappedScore, 60)
        }

        if (measurableImpactScore < 20) {
            cappedScore = minOf(cappedScore, 78)
        }
        ...
        if (
            ...
            cappedScore = minOf(cappedScore, 65)
        }

        val isSenior =
            experienceLevel.contains("Senior", ignoreCase = true) ||
                experienceLevel.contains("Lead", ignoreCase = true) ||
                experienceLevel.contains("7+") ||
                experienceLevel.contains("4-7")

        if (isSenior && measurableImpactScore < 40) {
            cappedScore = minOf(cappedScore, 70)
        }

        if (atsReadabilityScore < 40) {
            cappedScore = minOf(cappedScore, 75)
        }
...
${formatExamples(missing, "No major missing section detected.")}
    ...
    private fun resumeLines(text: String): List<String> {
        ...
        for (line in lines) {
            ...
            val isHeading = isResumeSectionHeading(line)
            val startsLowercase =
                line.firstOrNull()?.isLowerCase() == true
            val startsWithContinuationWord =
                continuationStarter.containsMatchIn(line)

            val previousIsHeading =
                currentBuffer.isNotEmpty() &&
                    isResumeSectionHeading(currentBuffer.toString())
    ...
    private fun isResumeSectionHeading(line: String): Boolean {
        ...
        return cleaned.lowercase() in commonHeadings ||
